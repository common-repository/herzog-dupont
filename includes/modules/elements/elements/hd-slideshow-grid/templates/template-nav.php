...
<?php echo $nav($props, $nav_attrs) ?>

    <?php if ($props['filter_all']) : ?>
    <li class="uk-active" uk-filter-control><a href="#"><?php echo $this->trans($props['filter_all_label'] ?: 'All') ?></a></li>
    <?php endif ?>

    <?php foreach ($tags as $tag => $name) : ?>
    <?php $selector = htmlspecialchars("[data-tag~='" . str_replace("'", "\'", $tag) . "']", ENT_QUOTES, null, false) ?>
    <li <?php echo $tag === key($tags) && !$props['filter_all'] ? 'class="uk-active" ' : '' ?>uk-filter-control="<?php echo $selector ?>">
